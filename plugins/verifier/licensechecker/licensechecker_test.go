...
func TestTempValidate(t *testing.T) {
	bash := utils.Package{
		PackageName:    "bash",
	...
	packageViolation, licensesViolation, _ := tempValidate(spdxTestBytes, disallowedLicenses, disallowedPackages)
	...
	// 3. when there is AND AND situation
	// 3. not found
}
